...
plt1 <- ggplot(data = new_usa_data, aes(x=long, y=lat, group = group, fill = grp.y)) +
  geom_polygon(color = "black") +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
...
# custom legend
plt2 <- plt1 + scale_fill_manual(name = NULL,
...
plt3 <- ggplot(circleDF, aes(x0 = x, y0 = y, r = 0.1, fill = group)) +
