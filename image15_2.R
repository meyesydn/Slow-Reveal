...
######
#Data with the coordinates for the Oklahoma split
OKdata <- read_excel("ok_data1.xlsx")

#plotting the map with Oklahoma split
OKplot <- ggplot(data = OKdata, aes(x=long, y=lat, group = group, fill = grp)) +
  geom_polygon(color = "black") +
  labs(title = "DENSITY OF NEGRO POPULATION IN 1890.") +
  theme(plot.title = element_text(hjust = 0.5, family = "Puritan", size = 17)) +
  theme(axis.title.x = element_blank(),
...
#Modifying the legend
OKplot2 <- OKplot + scale_fill_manual(name = NULL,
                                      labels = c("Less than 1 to sq. m,",
                                                 "1 - 4",
                                                 "4 - 8",
                                                 "8 - 15",
...
OKplot3 <- OKplot2 + with(new_centroids, 
                          annotate(geom = "text", x = long, y = lat, label = Abb, family = "Puritan"))
